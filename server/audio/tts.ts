...
// Default ElevenLabs voice. US (Daniel, American) covers every league. The
// legacy ELEVENLABS_VOICE_ID is honored as a fallback when the per-region var
// isn't set, so existing deployments keep their current voice.
...
// Resolve the ElevenLabs voice for a sport. All leagues read in the US voice.
export function getVoiceIdForSport(_sport: string): string {
  return usVoiceId();
}
...
// Generate (or return cached) speech. The voiceId is part of the cache hash, so
// the same script spoken by two voices yields distinct cached files. audioUrl
